...
describe Torta do
  ...
  describe "#status" do
    it "returns the torta's current status" do
      expect (torta.status).should eq("crudo")
    end
    ...
    context "when baked for less than 5 minutes" do
      it "is `crudo`" do 
        expect (torta.bake!(4).status).should eq("crudo")
      end
    end

    context "when baked for at least 5 but less than 7 minutes" do
      it "is `casi listo`" do
        expect (torta.bake!(6).status).should eq("casi listo")
      end
    end

    context "when baked for at least 7 but less or equal than 10 minutes" do
      it "is `listo`" do 
        expect (torta.bake!(8).status).should eq("listo")
      end
    end
  ...
  describe "#bake_time" do
    ...
    expect (torta.bake_time).should eq(10)
    end
  end
end
